...
3. **Assets**: Your logo and favicon (from the `[project]` `logo` and `favicon` paths in `chartbook.toml`, or package defaults) are copied into `_docs_src/_static/` and `_docs_src/assets/`.
...
- **Dataframe pages** (`dataframes/{pipeline_id}/{dataframe_id}.md`): Generated by rendering your user-authored dataframe documentation (from the dataframe's `docs_path` file, or inline `docs`) together with Jinja2 template partials for the heading, DataFrame Glimpse, dataframe manifest table, and pipeline manifest table.

- **Chart pages** (`charts/{pipeline_id}.{chart_id}.md`): Generated by combining `chart_entry_top.md` (ABlog frontmatter, heading, iframe embed) + your user-authored chart docs (from the chart's `docs_path` file, or inline `docs`) + `chart_entry_bottom.md` (specs table, manifest tables).

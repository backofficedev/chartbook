...
| Notebook `.ipynb` files | `cb/notebooks/{pipeline_id}/{notebook_id}.ipynb` | Rendered by myst-nb during Sphinx build |
...
- **Dataframe pages** (`cb/dataframes/{pipeline_id}/{dataframe_id}.md`): Generated by rendering your user-authored dataframe documentation (from the dataframe's `docs_path` file, or inline `docs`) together with Jinja2 template partials for the heading, DataFrame Glimpse, dataframe manifest table, and pipeline manifest table.
...
```markdown
# Dataframe: `repo_public` - Public Repo Data
